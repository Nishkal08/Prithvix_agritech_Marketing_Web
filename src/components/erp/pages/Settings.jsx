  ...
  return (
    <PageTransition>
      <div className="space-y-5 max-w-3xl">
          ...
          {['Profile', 'Subscription', 'Staff & Permissions'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
                activeTab === tab ? 'border-gold text-dark' : 'border-transparent text-muted hover:text-secondary'
              }`}>
              {tab}
            </button>
          ))}
        ...
        {/* Profile Tab */}
        {activeTab === 'Profile' && (
          <form onSubmit={handleSave} className="bg-white border border-[#E8E3DA] rounded-xl p-6 space-y-5">
            {/* Avatar */}
            <div className="flex items-center gap-4 pb-2 border-b border-[#F0EDE6]">
              <div className="relative group cursor-pointer">
                ...
                <div className="absolute inset-0 rounded-full bg-dark/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-[10px] text-white font-semibold">
                  Change
                </div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-[#E8E3DA] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold/50 bg-white"
                >
            ...
            <div className="flex justify-end">
              <button type="submit"
                ...
                {profileSaved && <Check size={16} className="text-[#1A3C2B]" />}
        ...
        {activeTab === 'Subscription' && (
          <div className="space-y-4">
            {/* Current Plan */}
            <div className="bg-white border border-[#E8E3DA] rounded-xl p-6 border-l-[4px] border-l-gold">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <span className="text-xs font-bold uppercase tracking-wider text-gold bg-[#F5E6C8] px-2.5 py-1 rounded-full">Current Plan</span>
              ...
              <div className="space-y-2.5 mb-6">
                {['Up to 500 farmers', '5 staff accounts', 'Udhaar tracking', 'Basic analytics', 'Email support'].map(feature => (
                  <div key={feature} className="flex items-center gap-2.5 text-sm text-dark">
                    <div className="w-5 h-5 rounded-full bg-[#D1E8DA] flex items-center justify-center">
                      <Check size={12} className="text-[#1A3C2B]" />
                    </div>
                    {feature}
                  </div>
                ))}
              </div>
              <button className="w-full bg-[#1A3C2B] text-offwhite font-semibold py-3 rounded-xl hover:bg-[#1A3C2B]/90 transition-colors">
                Upgrade to Premium →
              </button>
            </div>

            {/* Premium features preview */}
            <div className="bg-[#EDE8DF] border border-[#D6CFC3] rounded-xl p-6">
        ...
        {activeTab === 'Staff & Permissions' && (
          <div className="bg-white border border-[#E8E3DA] rounded-xl overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {staff.map((s, i) => (
                  <tr key={s.id} className={`border-b border-[#F5F0E8] last:border-0 ${i % 2 === 1 ? 'bg-[#FAFAF8]' : 'bg-white'}`} style={{ height: '52px' }}>
                    ...
                    <td className="px-4 py-2 text-secondary">{s.role}</td>
                    <td className="px-4 py-2 font-mono text-muted text-xs">{s.mobile}</td>
                    <td className="px-4 py-2"><StatusBadge status={s.status} /></td>
                    {user?.role === 'dealer' && (
                      <td className="px-4 py-2">
                        <button className="text-xs text-secondary hover:text-dark underline mr-3">Edit</button>
                        <button
