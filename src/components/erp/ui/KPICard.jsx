...
import { UserPlus, MapPin, TrendingUp, AlertCircle, Package } from 'lucide-react';
...
// Uses semantic tokens so dark mode works automatically
const ICON_COLORS = {
  up:   { bg: 'bg-forest/10', icon: 'text-forest' },
  warn: { bg: 'bg-gold/10',   icon: 'text-gold'   },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1">
        <span className={`text-[12px] font-medium ${trend === 'warn' ? 'text-gold' : 'text-green-500'}`}>
          {trend === 'up' ? '↑' : '⚠'} {delta}
        </span>
      </div>
    </motion.div>
